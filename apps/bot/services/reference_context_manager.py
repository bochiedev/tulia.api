...
class ReferenceContextManager:
    ...
    def resolve_reference(cls, conversation, message_text):
        """
        Resolve a positional reference to an actual item.
        ...
        Returns:
            dict with 'item', 'list_type', 'position' or None
        """
        ...
        if item:
            logger.info(
                f"Resolved reference '{message_text}' to position {display_position} "
    ...
    def _get_current_context(cls, conversation):
        ...
        try:
            return ReferenceContext.objects.filter(
                conversation=conversation,
